...
class ResourceSchedule:
    ...
    def init_schedule(self, resources):
        ...
        for resource_type in self.resource_types:
            ...
            number_of_resources_available_from_each_scheduling_moment = {
                0: self.resource_types[resource_type]
            }
            self.schedule[resource_type] = (
                scheduling_moments,
                number_of_resources_available_from_each_scheduling_moment,
            )
    ...
    def get_number_of_resources(self, resource_type, moment):
        ...
        return number_of_resources_available_from_each_scheduling_moment[
            scheduling_moments[scheduling_index]
        ]
    ...
    def get_current_resources(self, moment):
        ...
        for resource_type in self.schedule:
            current_resources[resource_type] = self.get_number_of_resources(
    ...
    def add_scheduling_moment(self, resource_type, moment, nr_available):
        ...
        if scheduling_moments[scheduling_index] == moment:
            number_of_resources_available_from_each_scheduling_moment[moment] = (
                nr_available
            )
        else:
    ...
    def add_cost_measurement(self, now, busy_resources):
        ...
        for resource_type in current_resources:
            cost += 2 * (
                current_resources[resource_type] - planned_ahead[resource_type]
            )
        ...
        for resource_type in busy_resource_count:
            cost += 3 * (
                max(
                    0,
                    busy_resource_count[resource_type]
...
class Simulator:
    def __init__(self, planner, problem):
        ...
        self.assigned_tasks = (
            dict()
        )  # dictionary of assigned tasks id -> (task, resource, moment of assignment)
        self.available_resources = set()  # set of available resources
        self.away_resources = (
            []
        )  # list of resources that are unavailable, because they are away
        self.busy_resources = (
            dict()
        )  # dictionary of busy resources resource -> (task they are busy on, moment they started on the task)
        self.busy_cases = (
            dict()
        )  # dictionary of busy cases case_id -> list of ids of elements that are planned in self.events for the case
        ...
        self.case_start_times = (
            dict()
        )  # dictionary of case_id -> moment the case started
    ...
    def sort_events(self):
        ...
        self.events.sort(
            key=lambda k: (
                k[0],  # time
                1 if k[1].event_type == EventType.COMPLETE_EVENT else 0,
            )
        )
    ...
    def init_simulation(self):
        ...
        self.events.append(
            (
                next_planning_moment,
                SimulationEvent(
                    EventType.REGULAR_PLANNING_MOMENT, next_planning_moment, None
                ),
            )
        )
    ...
    def cancel(self, case_id, event_label):
        ...
        for i in range(len(self.events)):
            if (
                self.events[i][1].element is not None
                and self.events[i][1].element.case_id == case_id
                and self.events[i][1].element.label == event_label
            ):
                found_index = i
        if found_index is not None:
            event = self.events.pop(found_index)
            # also remove the element from the busy case
            self.busy_cases[case_id] = list(
                filter(lambda k: k.id != event[1].element.id, self.busy_cases[case_id])
            )
    ...
    def is_planning_slot(self, time):
        ...
        hour_of_day_x_10 = round(
            (time_in_week % 24) * 10
        )  # we multiply by 10 to avoid floating point errors
        # day_of_week = round(time_in_week // 24)  # 0 is Monday, 1 is Tuesday, ..., 6 is Sunday
        day_of_week = True
        return (
            hour_of_day_x_10 >= 80
            and hour_of_day_x_10 <= 150
            and day_of_week < 5
            and hour_of_day_x_10 % 5 == 0
        )
    ...
    def replan(self, element, time):
        ...
        self.events.append(
            (time, SimulationEvent(EventType.COMPLETE_EVENT, time, element))
        )
        self.sort_events()
    ...
    def activate(self, element):
        ...
        if element.is_event():
            self.planner.report(
                element.case_id, element, self.now, None, EventType.ACTIVATE_EVENT
            )
            self.events.append(
                (
                    element.occurrence_time,
                    SimulationEvent(
                        EventType.COMPLETE_EVENT, element.occurrence_time, element
                    ),
                )
            )
        elif element.is_task():
            self.planner.report(
                element.case_id, element, self.now, None, EventType.ACTIVATE_TASK
            )
            self.unassigned_tasks[element.id] = element
            self.events.append(
                (self.now, SimulationEvent(EventType.ASSIGN_RESOURCES, self.now, None))
            )
        self.events.append(
            (self.now, SimulationEvent(EventType.PLAN_EVENTS, self.now, None))
        )
    ...
    def run(self, running_time=24 * 365):
        ...
        while self.now <= running_time:
            ...
            if event.event_type == EventType.CASE_ARRIVAL:
                self.planner.report(
                    event.element.case_id, None, self.now, None, EventType.CASE_ARRIVAL
                )  # report CASE_ARRIVAL
                # create the case
                ...
                t = self.now + self.problem.processing_time_sample(
                    event.resource, event.element, self.now
                )
                self.events.append(
                    (
                        t,
                        SimulationEvent(
                            EventType.COMPLETE_TASK, t, event.element, event.resource
                        ),
                    )
                )

            elif (
                event.event_type == EventType.COMPLETE_EVENT
                ...
                self.planner.report(
                    ...
                    event.resource,
                    event.event_type,
                )  # report COMPLETE_EVENT or COMPLETE_TASK
                ...
                if event.event_type == EventType.COMPLETE_TASK:
                    ...
                    if (
                        nr_resources_working_now[event.resource.type]
                        > nr_resources_scheduled_now[event.resource.type]
                    ):  # if there are too many resources of that type, send it away
                        ...
                        self.events.append(
                            (
                                self.now,
                                SimulationEvent(
                                    EventType.ASSIGN_RESOURCES, self.now, None
                                ),
                            )
                        )  # if a resource becomes available, it can be assigned, so we schedule the assignment of resources
                    del self.assigned_tasks[event.element.id]
                else:
                    self.event_times[event.element] = self.now

                # complete the element
                self.busy_cases[event.element.case_id] = list(
                    filter(
                ...
                if len(self.busy_cases[event.element.case_id]) == 0:
                    self.events.append(
                        (
                            self.now,
                            SimulationEvent(
                                EventType.COMPLETE_CASE, self.now, event.element
                            ),
                        )
                    )
                ...
                nr_resources_scheduled_now = self.schedule.get_current_resources(
                    self.now
                )
                ...
                for resource in self.away_resources:
                    if (
                        nr_resources_working_now[resource.type]
                        < nr_resources_scheduled_now[resource.type]
                    ):
                ...
                if len(resources_to_add) > 0:
                    self.events.append(
                        (
                            self.now,
                            SimulationEvent(EventType.ASSIGN_RESOURCES, self.now, None),
                        )
                    )  # if a resource becomes available, it can be assigned, so we schedule the assignment of resources
                ...
                self.events.append(
                    (
                        self.now + 1,
                        SimulationEvent(
                            EventType.SCHEDULE_RESOURCES, self.now + 1, None
                        ),
                    )
                )
                ...
                self.planner.report(
                    None, None, self.now, None, EventType.SCHEDULE_RESOURCES, data
                )  # report SCHEDULE_RESOURCES

            elif event.event_type == EventType.ASSIGN_RESOURCES:
                # assign resources to tasks
                if len(self.unassigned_tasks) > 0 and len(self.available_resources) > 0:
                    assignments = self.problem.assign_resources(
                        self.unassigned_tasks, self.available_resources
                    )
                    for task, resource in assignments:
                        self.events.append(
                            (
                                self.now,
                                SimulationEvent(
                                    EventType.START_TASK, self.now, task, resource
                                ),
                            )
                        )
                ...
                next_planning_moment = self.problem.next_regular_planning_moment(
                    self.now
                )
                self.events.append(
                    (
                        next_planning_moment,
                        SimulationEvent(
                            EventType.REGULAR_PLANNING_MOMENT,
                            next_planning_moment,
                            None,
                        ),
                    )
                ...
                if len(self.problem.can_plan) > 0:
                    ...
                    planned_events = self.planner.plan(
                        list(plannable_elements.keys()),
                        list(replannable_elements.keys()),
                        self.now,
                    )
                    for planned_element in planned_events:
                        if planned_element[0] in plannable_elements:
                            if len(plannable_elements[planned_element[0]]) != 1:
                                raise ValueError(
                                    "At this stage, we only allow for planning of one event at a time."
                                )
                            planned_element = self.problem.plan(
                                planned_element[0],
                                plannable_elements[planned_element[0]][0],
                                planned_element[1],
                            )
                            self.planned_events.append([self.now, planned_element])
                            if not planned_element.is_event():
                                raise ValueError(
                                    "At this stage, we only allow for planning of events."
                                )
                            ...
                            replanned_event = self.problem.plan(
                                case_id, element_label, time
                            )
                            ...
                            raise ValueError(
                                "Element not in plannable or replannable elements."
                            )

            elif event.event_type == EventType.COMPLETE_CASE:
                self.planner.report(
                    event.element.case_id, None, self.now, None, EventType.COMPLETE_CASE
                )  # report COMPLETE_CASE
                self.total_cycle_time += (
                    self.now - self.case_start_times[event.element.case_id]
                )
