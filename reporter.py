...
class Reporter:
    ...
    def get_formatted_timestamp(self, timestamp):
        return (self.initial_time + timedelta(hours=timestamp)).strftime(
            self.time_format
        )

    def callback(
        self, case_id, element, timestamp, resource, lifecycle_state, data=None
    ):
...
class EventLogReporter(Reporter):
    ...
    def __init__(self, filename, data_types):
        ...
        self.logfile.write(
            "case_id,task_id,event_label,resource,start_time,completion_time"
        )
    ...
    def callback(
        ...
        if lifecycle_state == EventType.START_TASK:
            self.task_start_times[(case_id, element.label)] = (
                self.get_formatted_timestamp(timestamp)
            )
        elif (
            lifecycle_state == EventType.COMPLETE_EVENT
            or lifecycle_state == EventType.COMPLETE_TASK
        ):
...
class ResourceScheduleReporter(Reporter):
    ...
    Has a function to create a graph of the available resources over time.
    """
    ...
    def __init__(self):
        ...
        self.away_resources = []
        self.time = []
        self.available_resources_list = []  # to find out WHICH resources are redundant.
    ...
    def callback(
        ...
        if lifecycle_state == EventType.SCHEDULE_RESOURCES:
            ...
            self.available_resources_list.append(
                data.get("available_resources_list", [])
            )

    def create_graph(self, start=0, end=None, name="resource_schedule.png"):
        """
        ...
        plt.fill_between(
            ...
            self.busy_resources[start:end],
            [
                self.available_resources[i] + self.busy_resources[i]
        ...
        plt.fill_between(
            ...
            [
                ...
                + self.busy_resources[i]
                + self.away_resources[i]
                for i in range(start, end)
            ],
            color="red",
            alpha=0.5,
        ...
        while start_week < end:
            start_week += 168
            plt.axvline(x=start_week, color="black", linestyle="--")

        start_day = start
        while start_day < end:
            start_day += 24
            plt.axvline(x=start_day, color="black", linestyle="--", linewidth=0.5)
